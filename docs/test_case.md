...
- [x] 用户消息换行展示验证（字满换行，无异常换行）
- [x] @采集专员前台深度采集验证（URL 提取、数据仓库保存、深度采集任务创建、表格卡片推送）
- [x] 历史记录卡片渲染持久化验证（weather/music/news/table 卡片重新渲染）
- [x] 左侧数字员工工具栏完整性验证（@天气/@随机音乐/@新闻/@文案写作助手/@小智/@采集专员）
- [x] 对话导出 PDF 功能验证（中文字体探测、异常捕获、越权校验）
- [x] 置顶会话防下沉验证（is_pinned DESC 排序）
